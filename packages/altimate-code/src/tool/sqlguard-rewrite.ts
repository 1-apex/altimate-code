...
export const SqlGuardRewriteTool = Tool.define("sqlguard_rewrite", {
  description:
    "Suggest query optimization rewrites using the Rust-based sqlguard engine. Analyzes SQL and proposes concrete rewrites for better performance.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.rewrite", {
        sql: args.sql,
